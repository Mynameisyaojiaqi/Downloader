...
def args_parse():
    ...
    parser.add_argument("--lat_start", type=float,  help='The starting latitude point')
    parser.add_argument("--lat_stop", type=float,  help='The ending latitude point')
    parser.add_argument('--lon_start', type=float, help='The starting longitude point')
    parser.add_argument('--lon_stop', type=float, help='The ending longitude point')
    parser.add_argument('--zoom', type=int, help='The zoom level to download')
    parser.add_argument('--outpath', type=str, help='Download path')
...
def main(left, top, right, bottom, zoom, filePath, style='s', server="Google"):
    ...
    #print("outpic的色彩模式为{}".format(outpic.mode))
    #outpic = pil.fromarray(outpic.astype(int), 'RGB')
    ...
    # print(extent['RB'][1])
    # print(extent['LT'][1])
    # print(abs(extent['RB'][1] - extent['LT'][1]))
    gt = (extent['LT'][0], abs(extent['RB'][0] - extent['LT'][0]) / r.shape[1], 0, extent['LT'][1], 0,
          abs(extent['RB'][1] - extent['LT'][1]) / r.shape[0])
    # print(gt)
    #gt = (extent['LT'][0], (extent['RB'][0] - extent['LT'][0]) / r.size[1], 0, extent['LT'][1], 0,(extent['RB'][1] - extent['LT'][1]) / r.size[0])
...
if __name__ == '__main__':
    ...
    #main(120.444595, 23.692934, 120.462222, 23.714939, 22, r'C:\Users\Yao\Downloads\test.tif', server="Google")
    main(args['lat_start'], args['lon_start'], args['lat_stop'], args['lon_stop'], args['zoom'], args['outpath'],style='s', server="Google")
    #main(lat_start, lon_start, lat_stop, lon_stop, zoom, outpath, server="Google")
